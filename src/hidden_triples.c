...
int eliminate_candidates_except_htriples(Cell *cell, int *candidates, int size) //array as a pointer = no need for []
   {        
    int counter = 0;
...
int eliminate_candidates_htriples(Cell **p_cells, int index1, int index2, int index3) {
    ...
    for (int i = 0; i < BOARD_SIZE; i++) {
        if (i != index1 && i != index2 && i != index3) {       //also eliminate in other cells in group
            counter += eliminate_candidates_except_htriples(p_cells[i], candidates, 3);      //with some exceptions
        }
    }
